...
def main():
    ...
    for idx, file_name in tqdm(enumerate(image_files)):
        ...
        if cfg.show_result or cfg.save_result or cfg.save_pc or cfg.show_pc:

            save_pc_path = output_dir / "alignment_pc" /  f"{frame_id:06d}.npy" if cfg.save_pc else None
            plot_3d(img_pil=img_pil, depth=depth, extr=extr, intr=intr, pred_boxes=waymo_boxes, show=cfg.show_pc, save_path=save_pc_path)
...
if __name__=="__main__":
    cfg = SimpleNamespace(
        ...
        show_result=True,  # plot the detection results
        save_result=False,  # save the above plot
        show_pc=False,  # show the detection in 3D using open3D
