...
class WaymoRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        crop, class_name, z_offset, extent_diff, heading_diff = next(self.iterator)
        ...
        return {
            ...
            'z_offset': torch.tensor(z_offset, dtype=torch.float32),
...
class WaymoRegressionIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        for crop, class_name, z_offset, extent_diff, heading_diff in iterator:
            if self.transform:
                crop = self.transform(crop)
...
if __name__ == "__main__":
    ...
    class_to_idx = {cls_name: idx for idx, cls_name in enumerate(class_names)}  # Add mapping for class names
    batch_size = 32
    num_epochs = 20
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), total=batches_per_epoch):
            ...
            offsets_gt = batch['z_offset'].to(device)  # (B, )
            ...
            # class_indices_batch = torch.tensor([class_to_idx[cls] for cls in class_names_batch]).to(device)

            # Forward pass for the entire batch
            preds = model(images, class_names_batch)
            offset_z_preds, extent_preds, heading_preds = preds[:, 0], preds[:, 1:4], preds[:, 4:6]

            # Compute the loss for the entire batch
            loss_offset = mse_loss(offset_z_preds, offsets_gt) / 80
        ...
        with torch.no_grad():
            for j, val_batch in enumerate(val_loader):
                ...
                val_offsets_gt = val_batch['z_offset'].to(device)
                ...
                val_offset_z_preds, val_extent_preds, val_heading_preds = val_preds[:, 0], val_preds[:, 1:4], val_preds[:, 4:6]

                val_loss_offset = mse_loss(val_offset_z_preds, val_offsets_gt) / 80
        ...
        print(f"Saved new best model to latest_{save_path}")
